...
(function() {
    ...
    // 🔒 PRODUCT CATALOG
    // IDs now match your HTML buttons AND your File Paths
    const PRODUCTS = {
        // === HERO OFFERS (Restored for HTML Compatibility) ===
        'prod_combo_complete': {
            id: 'prod_combo_complete',
            name: 'Complete Support Plan (Bundle)',
            price: 69.00,
            icon: '🔥',
            // Maps to the Total Bundle download for now
        ...
        'prod_prompt_executive': {
            ...
            price: 29.00,
            icon: '🤖',
            // Maps to Morning System for now
        ...
        // === TIER 1: ACTIVITY PACKETS (IDs Updated to match Folders) ===
        // Note: Assuming these zips exist in these locations based on your structure
        'prod-packet-bravely': { id: 'prod-packet-bravely', name: 'Activity Pack: Bravely', price: 9.00, icon: '🦁', downloadUrl: '/downloads/activity-packets/bravely-the-lion.zip' },
        'prod-packet-cosmo':   { id: 'prod-packet-cosmo', name: 'Activity Pack: Cosmo', price: 9.00, icon: '🚀', downloadUrl: '/downloads/activity-packets/cosmo-space-mission.zip' },
        'prod-packet-ember':   { id: 'prod-packet-ember', name: 'Activity Pack: Ember', price: 9.00, icon: '🔥', downloadUrl: '/downloads/activity-packets/ember-the-dragon.zip' },
        'prod-packet-shelly':  { id: 'prod-packet-shelly', name: 'Activity Pack: Shelly', price: 9.00, icon: '🐢', downloadUrl: '/downloads/activity-packets/shelly-the-turtle.zip' },
        'prod-packet-sketch':  { id: 'prod-packet-sketch', name: 'Activity Pack: Sketch', price: 9.00, icon: '🦉', downloadUrl: '/downloads/activity-packets/sketch-the-owl.zip' },
        'prod-packet-whisper': { id: 'prod-packet-whisper', name: 'Activity Pack: Whisper', price: 9.00, icon: '🐰', downloadUrl: '/downloads/activity-packets/whisper-the-cozy.zip' }
    };

    // ... (Rest of the Cart Class logic remains the same) ...
    class Cart {
        constructor() {
        ...
        dispatchUpdate() {
            window.dispatchEvent(new CustomEvent('cartUpdated', {
                detail: { items: this.items, count: this.getItemCount(), total: this.getTotal() }
            }));
        }

        formatCurrency(amount) {
            return new Intl.NumberFormat('en-US', { style: 'currency', currency: CART_CONFIG.currency }).format(amount);
        }
        ...
        add(productId, quantity = 1) {
            ...
            if (existingIndex > -1) {
                ...
                this.items.push({ id: productId, quantity: quantity, addedAt: new Date().toISOString() });
            }
        ...
        getItems() {
            return this.items.map(item => ({ ...item, product: PRODUCTS[item.id] })).filter(item => item.product);
        }
        ...
        showAddedNotification(product) {
            ...
            Object.assign(notification.style, {
                position: 'fixed', bottom: '20px', right: '20px', background: '#2D3748',
        ...
        if (cart.isEmpty()) {
            container.innerHTML = `
                <div class="text-center py-12">
                    ...
                    <a href="/products/" class="inline-block bg-[#FF6B6B] text-white px-6 py-3 rounded-xl font-bold mt-4">Browse Tools</a>
                </div>`;
            return;
        }
        ...
        const itemsHTML = cart.getItems().map(item => `
            <div class="flex items-center justify-between border-b border-gray-100 py-6">
                <div class="flex items-center gap-4">
                    <div class="w-16 h-16 bg-gray-50 rounded-lg flex items-center justify-center text-3xl">${item.product.icon}</div>
                    <div>
                        <h4 class="font-bold text-gray-800">${item.product.name}</h4>
                        <p class="text-sm text-gray-500">$${item.product.price.toFixed(2)}</p>
                    </div>
                </div>
                <button onclick="window.NavigatorCart.remove('${item.id}')" class="text-gray-400 hover:text-red-500 px-3"><i class="fa-solid fa-trash"></i></button>
            </div>
        `).join('');
        ...
        container.innerHTML = `
            <div class="grid md:grid-cols-3 gap-8">
                <div class="md:col-span-2">
                    <div class="bg-white rounded-2xl shadow-sm p-6 mb-6">${itemsHTML}</div>
                    <div class="text-right"><button onclick="window.NavigatorCart.clear()" class="text-sm text-red-400 underline">Empty Cart</button></div>
                </div>
                <div class="md:col-span-1">
                    <div class="cart-summary bg-white p-6 rounded-2xl shadow-sm border border-gray-100 sticky top-24">
                        ...
                        <button onclick="window.NavigatorCart.checkout()" class="w-full bg-[#FF6B6B] text-white font-bold py-4 rounded-xl shadow-lg">Checkout Securely</button>
                    </div>
                </div>
            </div>
        `;
