...
def fix_fonts(html: str) -> str:
    ...
    # Add new fonts if not already present
    if 'Fraunces' not in html:
        html = html.replace('</head>', f'{FONT_LINK}\n</head>', 1)
    return html


def fix_dead_links(html: str) -> str:
    """Update nav links that point to deprecated pages."""
    replacements = [
        # Products -> pricing
        ...
        # IEP battle plan is fine, keep it
        # Old quiz button in nav -> app
...
def add_cta_block(html: str, district_slug: str) -> str:
    """Append district-specific CTA before </body> if not already present."""
    if 'Start tracking' in html:
        return html  # already has a CTA
...
def main():
    ...
    for d in target_dirs:
        ...
        maps_note = f'  {report["maps_removed"]} map(s) removed' if report['maps_removed'] > 0 else ''
        print(f'  {status:<16} {label} ({neighborhoods}){maps_note}')
